...
BOOST_AUTO_TEST_CASE(doubleD1Q3) {
  ...
  const auto c0_r = MathVector<double, dimD_>{{0.0}};
  BOOST_CHECK_EQUAL(c0_, c0_r);
  const auto c1_ = P::celerity()[1];
  const auto c1_r = MathVector<double, dimD_>{{-1.0}};
  BOOST_CHECK_EQUAL(c1_, c1_r);
  const auto c2_ = P::celerity()[2];
  const auto c2_r = MathVector<double, dimD_>{{1.0}};
...
BOOST_AUTO_TEST_CASE(doubleD2Q9) {
  ...
  const auto c0_ = P::celerity()[0];
  const auto c0_r = MathVector<double, dimD_>{{0.0, 0.0}};
  BOOST_CHECK_EQUAL(c0_, c0_r);
  const auto c1_ = P::celerity()[1];
  const auto c1_r = MathVector<double, dimD_>{{-1.0, 1.0}};
  BOOST_CHECK_EQUAL(c1_, c1_r);
  const auto c2_ = P::celerity()[2];
  const auto c2_r = MathVector<double, dimD_>{{-1.0, 0.0}};
  BOOST_CHECK_EQUAL(c2_, c2_r);
  const auto c3_ = P::celerity()[3];
  const auto c3_r = MathVector<double, dimD_>{{-1.0, -1.0}};
  BOOST_CHECK_EQUAL(c3_, c3_r);
  const auto c4_ = P::celerity()[4];
  const auto c4_r = MathVector<double, dimD_>{{0.0, -1.0}};
  BOOST_CHECK_EQUAL(c4_, c4_r);
  const auto c5_ = P::celerity()[5];
  const auto c5_r = MathVector<double, dimD_>{{1.0, -1.0}};
  BOOST_CHECK_EQUAL(c5_, c5_r);
  const auto c6_ = P::celerity()[6];
  const auto c6_r = MathVector<double, dimD_>{{1.0, 0.0}};
  BOOST_CHECK_EQUAL(c6_, c6_r);
  const auto c7_ = P::celerity()[7];
  const auto c7_r = MathVector<double, dimD_>{{1.0, 1.0}};
  BOOST_CHECK_EQUAL(c7_, c7_r);
  const auto c8_ = P::celerity()[8];
  const auto c8_r = MathVector<double, dimD_>{{0.0, 1.0}};
