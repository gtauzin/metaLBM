  ...
  ForwardFFT(double* localSpacePtr_in,
    ...
    for (auto iC = 0; iC < NumberComponents; ++iC) {
      planForward[iC] = fftw_mpi_plan_dft_r2c(
          Dimension, globalLength_in, localSpacePtr + numberElements_in * iC,
  ...
  BackwardFFT(double* localFourierPtr_in,
    ...
    for (auto iC = 0; iC < NumberComponents; ++iC) {
      planBackward[iC] = fftw_mpi_plan_dft_c2r(
          ...
          (fftw_complex*)(localFourierPtr + numberElements_in * iC),
          localSpacePtr + numberElements_in * iC, MPI_COMM_WORLD,
          FFTW_ESTIMATE);
    }
  }
...
class Curl<double, Architecture::CPU, PartitionningType::OneD, 2, 2> {
 ...
 public:
  Curl(double* localSpaceInPtr_in,
      ...
      : forwardIn(localSpaceInPtr_in,
                  localSpaceInPtr_in,
                  numberElements_in,
                  globalLength_in),
        backwardIn(localSpaceInPtr_in,
                   localSpaceInPtr_in,
                   numberElements_in,
                   globalLength_in),
        backwardOut(localSpaceOutPtr_in,
                    localSpaceOutPtr_in,
                    numberElements_in,
                    globalLength_in),
        numberElements(numberElements_in),
    ...
    computationFourier.Do([=] LBM_HOST(const Position& iFP) {
      ...
      ((fftw_complex*)(backwardOut.localFourierPtr))[index][p::Re] =
          ...
          iK[d::Y] * ((fftw_complex*)(forwardIn.localFourierPtr +
                                      numberElements * (d::X)))[index][p::Im];

      ((fftw_complex*)(backwardOut.localFourierPtr))[index][p::Im] =
          iK[d::X] * ((fftw_complex*)(forwardIn.localFourierPtr +
                                      numberElements * (d::Y)))[index][p::Re] -
          iK[d::Y] * ((fftw_complex*)(forwardIn.localFourierPtr +
...
template <>
class MakeIncompressible<double,
                         Architecture::CPU,
                         PartitionningType::OneD,
                         2> {
 private:
