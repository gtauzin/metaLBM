...
namespace lbm {
  ...
  public:
    LBM_HOST LBM_DEVICE
    EntropicStepFunctor(const MathVector<T, L::dimQ>& f_Forced_in,
                        const MathVector<T, L::dimQ>& f_NonEq_in)
      : f_Forced(f_Forced_in)
      , f_NonEq(f_NonEq_in)
    {}
    ...
    inline T evaluateFunction(T const& alpha) {
      ...
      for(auto iQ = 0; iQ < L::dimQ; ++iQ) {
        T fmAlphafNeq_iQ = f_Forced[iQ] - alpha*f_NonEq[iQ];
        entropicStepFunction += f_Forced[iQ]*log(f_Forced[iQ]/L::weight()[iQ])
    ...
    inline T evaluateDerivative(T const& alpha) {
      ...
      for(auto iQ = 0; iQ < L::dimQ; ++iQ) {
        ...
        entropicStepFunctionDerivative +=
          f_NonEq[iQ]*(1 + log(fmAlphafNeq_iQ/L::weight()[iQ]));
      }
  ...
  template <class T>
    LBM_HOST LBM_DEVICE inline bool NewtonRaphsonSolver(
        EntropicStepFunctor<T> functor, const T tolerance,
        const int iterationMax, T& xR, const T xMin, const T xMax) {
    T error = 1 + tolerance;
    T xStep = 0.0;
  ...
  template <class T>
    LBM_HOST LBM_DEVICE inline bool Bisection_NewtonRaphsonSolver(
        EntropicStepFunctor<T> functor, const T tolerance,
        const int iterationMax, T& xR, const T xMin, const T xMax) {
    T xLow, xHigh;
    ...
    for (int iteration = 1; iteration <= iterationMax; ++iteration) {
      if ((((xR - xHigh) * derivativeEvaluation - functionEvaluation) *
           ((xR - xLow) * derivativeEvaluation - functionEvaluation) >
           0.0) ||
          (fabs(2.0 * functionEvaluation) >
           fabs(xStepPrevious * derivativeEvaluation))) {
        xStepPrevious = xStep;
