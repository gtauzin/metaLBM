 ...
 public:
  ...
  LBM_HOST LBM_DEVICE inline T evaluateFunction(T const& alpha) {
    ...
    for (auto iQ = 0; iQ < L::dimQ; ++iQ) {
      T f_iQ = haloDistributionNext_Ptr[hSD::getIndex(iP, iQ)];
      T fmAlphafNeq_iQ =
          f_iQ - alpha * haloDistributionPrevious_Ptr[hSD::getIndex(
  ...
  LBM_HOST LBM_DEVICE inline T evaluateDerivative(T const& alpha) {
    ...
    for (auto iQ = 0; iQ < L::dimQ; ++iQ) {
      T fNeq_iQ = haloDistributionPrevious_Ptr[hSD::getIndex(
          iP - uiL::celerity()[iQ], iQ)];
      T fmAlphafNeq_iQ =
          haloDistributionNext_Ptr[hSD::getIndex(iP, iQ)] - alpha * fNeq_iQ;
